...
### gateway-server
This CCIP-read gateway server (deployed on AWS) picks up mempool TXs and lifts runes to EVM chains. We attempted two ways to pick up BTC TXs (i.e., using [btc-worker scripts](https://github.com/memlayeronbtc/memlayer-btc2024/blob/main/btc-workers/depositWorker.js) on a full node or `mempool.space websocket API`). The gateway server signs the BTC data with the same private key that deploys the ERC-20 rune contracts for authenticity. This gateway server does not store any user or TX data.
